<?php
namespace App;

use Illuminate\Database\Eloquent\Model;
use Carbon\Carbon;
use Illuminate\Database\Eloquent\SoftDeletes;

/**
 * Class Vacation
 *
 * @package App
 * @property integer $employee_code
 * @property string $request_date
 * @property string $start_date
 * @property string $end_date
 * @property integer $count_days
 * @property string $vacation_type
 * @property text $reason
 * @property string $employee_alternative
 * @property tinyInteger $accepted_by_manager
 * @property tinyInteger $accepted_by_hr
*/
class Vacation extends Model
{
    use SoftDeletes;

    protected $fillable = ['employee_code', 'request_date', 'start_date', 'end_date', 'count_days', 'reason', 'accepted_by_manager', 'accepted_by_hr', 'vacation_type_id', 'employee_alternative_id'];
    
    public static function boot()
    {
        parent::boot();

        Vacation::observe(new \App\Observers\UserActionsObserver);
    }

    /**
     * Set attribute to money format
     * @param $input
     */
    public function setEmployeeCodeAttribute($input)
    {
        $this->attributes['employee_code'] = $input ? $input : null;
    }

    /**
     * Set attribute to date format
     * @param $input
     */
    public function setRequestDateAttribute($input)
    {
        if ($input != null && $input != '') {
            $this->attributes['request_date'] = Carbon::createFromFormat(config('app.date_format'), $input)->format('Y-m-d');
        } else {
            $this->attributes['request_date'] = null;
        }
    }

    /**
     * Get attribute from date format
     * @param $input
     *
     * @return string
     */
    public function getRequestDateAttribute($input)
    {
        $zeroDate = str_replace(['Y', 'm', 'd'], ['0000', '00', '00'], config('app.date_format'));

        if ($input != $zeroDate && $input != null) {
            return Carbon::createFromFormat('Y-m-d', $input)->format(config('app.date_format'));
        } else {
            return '';
        }
    }

    /**
     * Set attribute to date format
     * @param $input
     */
    public function setStartDateAttribute($input)
    {
        if ($input != null && $input != '') {
            $this->attributes['start_date'] = Carbon::createFromFormat(config('app.date_format'), $input)->format('Y-m-d');
        } else {
            $this->attributes['start_date'] = null;
        }
    }

    /**
     * Get attribute from date format
     * @param $input
     *
     * @return string
     */
    public function getStartDateAttribute($input)
    {
        $zeroDate = str_replace(['Y', 'm', 'd'], ['0000', '00', '00'], config('app.date_format'));

        if ($input != $zeroDate && $input != null) {
            return Carbon::createFromFormat('Y-m-d', $input)->format(config('app.date_format'));
        } else {
            return '';
        }
    }

    /**
     * Set attribute to date format
     * @param $input
     */
    public function setEndDateAttribute($input)
    {
        if ($input != null && $input != '') {
            $this->attributes['end_date'] = Carbon::createFromFormat(config('app.date_format'), $input)->format('Y-m-d');
        } else {
            $this->attributes['end_date'] = null;
        }
    }

    /**
     * Get attribute from date format
     * @param $input
     *
     * @return string
     */
    public function getEndDateAttribute($input)
    {
        $zeroDate = str_replace(['Y', 'm', 'd'], ['0000', '00', '00'], config('app.date_format'));

        if ($input != $zeroDate && $input != null) {
            return Carbon::createFromFormat('Y-m-d', $input)->format(config('app.date_format'));
        } else {
            return '';
        }
    }

    /**
     * Set attribute to money format
     * @param $input
     */
    public function setCountDaysAttribute($input)
    {
        $this->attributes['count_days'] = $input ? $input : null;
    }

    /**
     * Set to null if empty
     * @param $input
     */
    public function setVacationTypeIdAttribute($input)
    {
        $this->attributes['vacation_type_id'] = $input ? $input : null;
    }

    /**
     * Set to null if empty
     * @param $input
     */
    public function setEmployeeAlternativeIdAttribute($input)
    {
        $this->attributes['employee_alternative_id'] = $input ? $input : null;
    }
    
    public function vacation_type()
    {
        return $this->belongsTo(VacationType::class, 'vacation_type_id')->withTrashed();
    }
    
    public function employee_alternative()
    {
        return $this->belongsTo(User::class, 'employee_alternative_id');
    }
    
}
